...
class StrreplyController extends Controller
{
  ...
  public function replyviews(){
    ...
    if(Auth::user()->role_idrole == 2){
      ...
      return view('stronlines.rpreplyviews', compact('replies', 'idreceivers'));
    }else{
  ...
  public function replystore(){
    ...
    $idupdates = DB::table('str_form')->select('str_no')->where('reporter_idusr', '=', $sendto)->where('str_stt', '=', '1')->get();
    $idupd = '';
    foreach($idupdates as $idupdate){
      if($idupd == ''){
        $idupd = $idupdate->str_no;
      }else{
        $idupd = $idupd . '_' . $idupdate->str_no;
      }
    }
