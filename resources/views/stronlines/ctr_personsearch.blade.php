...
<table class="table ">
			...
			@if($ctrpersonstats->count())
				@foreach($ctrpersonstats as $ctrpersonstat)
				<tr>
					<td>{{ $ctrpersonstat->id }}</td>
					<td>{{ $ctrpersonstat->name }}</td>
					<td>{{ $ctrpersonstat->identity_card }}</td>
        ...
        <div>
            <b>{{ 'ການຄົ້ນຫາມີທັງຫມົດ '. number_format($ctrpersonstats->total()) .' ລາຍຊື່ທີ່ໃກ້ຄຽງ'}}</b>
        </div>

        <div class="d-flex justify-content-center">
        
            {{ $ctrpersonstats->appends(request()->except('page'))->links() }}
        </div>
