...
<div class="row">
  ...
  <form  class="form-horizontal" method="POST" action={{ url('/commentshow')}} role="form" enctype="multipart/form-data" accept-charset="UTF-8">
  {{ csrf_field() }}
      ...
      <div style="background-color: #eee;">
        <table id="dynamic-table" class="table table-striped table-bordered table-hover">
          ...
          @endphp
            ...
            @if(isset($commentreports))
                @foreach($commentreports as $commentreport)
                  <tr>
                    <td class="center">{{$i }}</td>
        						<td>{{ $commentreport->comment_form_date }}</td>
                    <td>{{ $commentreport->comment_form_detail }}</td>
